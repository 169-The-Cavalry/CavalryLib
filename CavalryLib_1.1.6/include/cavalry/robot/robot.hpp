...
#include "cavalry/asset.hpp"
#include "cavalry/robot/trackingWheel.hpp"
#include "cavalry/pose.hpp"
#include "cavalry/pid.hpp"
#include "cavalry/exitcondition.hpp"
#include "cavalry/driveCurve.hpp"
...
class OdomSensors {
    public:
        /**
         * The sensors are stored in a class so that they can be easily passed to the robot class
         * The variables are pointers so that they can be set to nullptr if they are not used
         * Otherwise the robot class would have to have a constructor for each possible combination of sensors
         *
         ...
         * cavalry::TrackingWheel vertical1(&vertical_rotation, cavalry::Omniwheel::NEW_275, 0.5);
         * cavalry::OdomSensors sensors(&vertical1, // vertical tracking wheel
...
/**
 * @brief class containing constants for a robot controller
 */
class ControllerSettings {
    public:
         ...
         *
         * The constants are stored in a class so that they can be easily passed to the robot class
         ...
         * @param smallError range of error at which the robot controller will exit if the error is within this range
         * for an amount of time determined by smallErrorTimeout
         * @param smallErrorTimeout the time the robot controller will wait before exiting if error is within a
         * certain range determined by smallError
         * @param largeError range of error at which the robot controller will exit if the error is within this range
         * for an amount of time determined by largeErrorTimeout
         * @param largeErrorTimeout the time the robot controller will wait before exiting if error is within a
         * certain range determined by largeError
         * @param slew maximum acceleration
         *
         * @b Example
         * @code {.cpp}
         * cavalry::ControllerSettings lateralSettings(10, // proportional gain (kP)
...
class Drivetrain {
    public:
         ...
         *
         * cavalry::Drivetrain drivetrain(&leftMotors, // left motor group
         *                               &rightMotors, // right motor group
         *                               10, // 10 inch track width
         *                               cavalry::Omniwheel::NEW_4, // using new 4" omnis
...
/**
 * @brief Parameters for robot::turnToPoint
 *
 * We use a struct to simplify customization. robot::turnToPoint has many
...
/**
 * @brief Parameters for robot::turnToHeading
 *
 * We use a struct to simplify customization. robot::turnToHeading has many
 * parameters and specifying them all just to set one optional param ruins
 * readability. By passing a struct to the function, we can have named
 * parameters, overcoming the c/c++ limitation
 */
...
/**
 * @brief Parameters for robot::swingToPoint
 *
 * We use a struct to simplify customization. robot::swingToPoint has many
...
/**
 * @brief Parameters for robot::swingToHeading
 *
 * We use a struct to simplify customization. robot::swingToHeading has many
 * parameters and specifying them all just to set one optional param harms
 * readability. By passing a struct to the function, we can have named
 * parameters, overcoming the c/c++ limitation
 */
...
/**
 * @brief Parameters for robot::moveToPose
 *
 * We use a struct to simplify customization. robot::moveToPose has many
 * parameters and specifying them all just to set one optional param ruins
 * readability. By passing a struct to the function, we can have named
 * parameters, overcoming the c/c++ limitation
 */
...
/**
 * @brief Parameters for robot::moveToPoint
 *
 * We use a struct to simplify customization. robot::moveToPoint has many
 * parameters and specifying them all just to set one optional param harms
 * readability. By passing a struct to the function, we can have named
 * parameters, overcoming the c/c++ limitation
 */
...
class Robot {
    public:
         ...
         *
         * @param drivetrain drivetrain to be used for the robot
         ...
         * @example main.cpp
         */
        Robot(Drivetrain drivetrain, ControllerSettings linearSettings, ControllerSettings angularSettings,
        ...
        /**
         * @brief Calibrate the robot sensors. THis should be called in the initialize function
         *
         ...
         * @b Example
         * @code {.cpp}
         * // set the pose of the robot to x = 0, y = 0, theta = 0
         * robot.setPose(0, 0, 0);
         * // set the pose of the robot to x = 5.3, y = 12.2, theta = 3.14
         * // this time with theta in radians
         * robot.setPose(5.3, 12.2, 3.14, true);
         * @endcode
         */
         ...
         * @code {.cpp}
         * // set the pose of the robot to x = 0, y = 0, theta = 0
         * cavalry::Pose poseA(0, 0, 0);
         * robot.setPose(poseA);
         * // set the pose of the robot to x = 5.3, y = 12.2, theta = 3.14
         * // this time with theta in radians
         * cavalry::Pose poseB(5.3, 12.2, 3.14);
         * robot.setPose(poseB, true);
         * @endcode
         */
         ...
         * cavalry::Pose pose = robot.getPose();
         * // print the x, y, and theta values of the pose
         * printf("X: %f, Y: %f, Theta: %f\n", pose.x, pose.y, pose.theta);
         * // get the pose of the robot in radians
         * cavalry::Pose poseRad = robot.getPose(true);
         * // print the x, y, and theta values of the pose
         * printf("X: %f, Y: %f, Theta: %f\n", poseRad.x, poseRad.y, poseRad.theta);
         * // get the pose of the robot in radians and standard position
         * cavalry::Pose poseRadStandard = robot.getPose(true, true);
         * // print the x, y, and theta values of the pose
         ...
         * robot.moveToPose(20, 15, 90, 4000);
         ...
         * robot.setBrakeMode(pros::E_MOTOR_BRAKE_BRAKE);
         * @endcode
         */
        void setBrakeMode(pros::motor_brake_mode_e mode);
        /**
         * @brief Turn the robot so it is facing the target point
         *
         * @param x x location
         * @param y y location
         ...
         * robot.turnToPoint(7.5, 7.5, 2000, {.minSpeed = 60, .earlyExitRange = 5});
         * @endcode
         */
        void turnToPoint(float x, float y, int timeout, TurnToPointParams params = {}, bool async = true);
        /**
         * @brief Turn the robot so it is facing the target heading
         *
         ...
         * // and a maximum speed of 60
         * robot.turnToHeading(230.5, 2000, {.maxSpeed = 60});
         * // turn the robot to face heading -90 with a timeout of 1500ms
         * // and turn counterclockwise
         * robot.turnToHeading(-90, 1500, {.direction = AngularDirection::CCW_COUNTERCLOCKWISE});
         * // turn the robot to face heading 90 with a timeout of 500ms
         * // with a minSpeed of 20 and a maxSpeed of 60
         * robot.turnToHeading(90, 500, {.maxSpeed = 60, .minSpeed = 20});
         * // turn the robot to face heading 45 with a timeout of 2000ms
         * // and a minSpeed of 60, and exit the movement if the robot is within 5 degrees of the target
         * robot.turnToHeading(45, 2000, {.minSpeed = 60, .earlyExitRange = 5});
         * @endcode
         */
        void turnToHeading(float theta, int timeout, TurnToHeadingParams params = {}, bool async = true);
        /**
         * @brief Turn the robot so it is facing the target heading, but only by moving one half of the drivetrain
         *
         * @param theta heading location
         ...
         * robot.swingToHeading(135, DriveSide::LEFT, 1000);
         * // turn the robot to face heading 230.5 with a timeout of 2000ms
         * // and a maximum speed of 60
         * // and lock the right side of the drivetrain
         * robot.swingToHeading(230.5, DriveSide::RIGHT, 2000, {.maxSpeed = 60});
         * // turn the robot to face heading -90 with a timeout of 1500ms
         * // and turn counterclockwise
         * // and lock the left side of the drivetrain
         * robot.swingToHeading(-90, DriveSide::LEFT, 1500, {.direction = AngularDirection::CCW_COUNTERCLOCKWISE});
         * // turn the robot to face heading 90 with a timeout of 500ms
         * // with a minSpeed of 20 and a maxSpeed of 60
         * // and lock the right side of the drivetrain
         * robot.swingToHeading(90, DriveSide::RIGHT, 500, {.maxSpeed = 60, .minSpeed = 20});
         * // turn the robot to face heading 45 with a timeout of 2000ms
         * // and a minSpeed of 60, and exit the movement if the robot is within 5 degrees of the target
         * // and lock the left side of the drivetrain
         * robot.swingToHeading(45, DriveSide::LEFT, 2000, {.minSpeed = 60, .earlyExitRange = 5});
         * @endcode
         */
        void swingToHeading(float theta, DriveSide lockedSide, int timeout, SwingToHeadingParams params = {},
                            bool async = true);
        /**
         * @brief Turn the robot so it is facing the target point, but only by moving one half of the drivetrain
         *
         * @param x x location
         * @param y y location
         * @param lockedSide side of the drivetrain that is locked
         ...
         * // turn the robot to face the point x = 45, y = -45, with a timeout of 1000ms
         * // and lock the left side of the drivetrain
         * robot.swingToPoint(45, -45, DriveSide::LEFT, 1000);
         * // turn the robot to face the point x = 45, y = -45, with a timeout of 1000ms
         * // but face the point with the back of the robot
         * // and lock the right side of the drivetrain
         * robot.swingToPoint(45, -45, DriveSide::RIGHT, 1000, {.forwards = false});
         * // turn the robot to face the point x = -20, 32.5 with a timeout of 2000ms
         * // and a maximum speed of 60
         * // and lock the left side of the drivetrain
         * robot.swingToPoint(-20, 32.5, DriveSide::LEFT, 2000, {.maxSpeed = 60});
         * // turn the robot to face the point x = -30, y = 22.5 with a timeout of 1500ms
         * // and turn counterclockwise
         * // and lock the right side of the drivetrain
         * robot.swingToPoint(-30, 22.5, DriveSide::RIGHT, 1500, {.direction =
         * AngularDirection::CCW_COUNTERCLOCKWISE});
         * // turn the robot to face the point x = 10, y = 10 with a timeout of 500ms
         * // with a minSpeed of 20 and a maxSpeed of 60
         * // and lock the left side of the drivetrain
         * robot.swingToPoint(10, 10, DriveSide::LEFT, 500, {.maxSpeed = 60, .minSpeed = 20});
         * // turn the robot to face the point x = 7.5, y = 7.5 with a timeout of 2000ms
         * // and a minSpeed of 60, and exit the movement if the robot is within 5 degrees of the target
         * // and lock the right side of the drivetrain
         * robot.swingToPoint(7.5, 7.5, DriveSide::RIGHT, 2000, {.minSpeed = 60, .earlyExitRange = 5});
         * @endcode
         */
        void swingToPoint(float x, float y, DriveSide lockedSide, int timeout, SwingToPointParams params = {},
                          bool async = true);
        /**
         * @brief Move the robot towards the target pose
         *
         ...
         * // move the robot to x = 20, y = 15, and face heading 90 with a timeout of 4000ms
         * // but face the point with the back of the robot
         * robot.moveToPose(20, 15, 90, 4000, {.forwards = false});
         * // move the robot to x = -20, 32.5 and face heading 90 with a timeout of 4000ms
         * // with a maxSpeed of 60
         * robot.moveToPose(-20, 32.5, 90, 4000, {.maxSpeed = 60});
         * // move the robot to x = 10, y = 10 and face heading 90
         * // with a minSpeed of 20 and a maxSpeed of 60
         * robot.moveToPose(10, 10, 90, 4000, {.maxSpeed = 60, .minSpeed = 20});
         ...
         * robot.moveToPose(7.5, 7.5, 90, 4000, {.minSpeed = 60, .earlyExitRange = 5});
         ...
         * robot.moveToPoint(20, 15, 4000);
         * // move the robot to x = 20, y = 15 with a timeout of 4000ms
         * // but face the point with the back of the robot
         * robot.moveToPoint(20, 15, 4000, {.forwards = false});
         * // move the robot to x = -20, 32.5 with a timeout of 4000ms
         * // with a maxSpeed of 60
         * robot.moveToPoint(-20, 32.5, 4000, {.maxSpeed = 60});
         * // move the robot to x = 10, y = 10 with a timeout of 4000ms
         * // with a minSpeed of 20 and a maxSpeed of 60
         * robot.moveToPoint(10, 10, 4000, {.maxSpeed = 60, .minSpeed = 20});
         * // move the robot to x = 7.5, y = 7.5 with a timeout of 4000ms
         * // with a minSpeed of 60, and exit the movement if the robot is within 5 inches of the target
         * robot.moveToPoint(7.5, 7.5, 4000, {.minSpeed = 60, .earlyExitRange = 5});
         * @endcode
         */
         ...
         *     robot.follow(myPath_txt, 10, 4000);
         *     // follow the path in "myPath.txt" with a lookahead of 10 inches and a timeout of 4000ms
         *     // but follow the path backwards
         *     robot.follow(myPath_txt, 10, 4000, false);
         * }
         * @endcode
         */
         ...
         *         // move the robot with tank controls
         ...
         *         // move the robot with arcade controls
         *         robot.arcade(leftY, // throttle
         *                        rightX, // steer
         ...
         *         int leftY = controller.get_analog(pros::E_CONTROLLER_ANALOG_LEFT_Y);
         *         int rightX = controller.get_analog(pros::E_CONTROLLER_ANALOG_RIGHT_X);
         *         // move the robot with curvature controls
         *         robot.curvature(leftY, rightX);
         *         // delay to save resources
         *         pros::delay(25);
         *     }
         * }
         ...
         * robot.moveToPoint(10, 10, 4000);
         * @endcode
         */
        void cancelMotion();
        /**
         * @brief Cancels all motions, even those that are queued.
         * After this, the robot will not be in motion.
         *
         * @b Example
         * @code {.cpp}
         * // move the robot to x = 20, y = 20 with a timeout of 4000ms
         ...
         * robot.setPose(10, 15, 90);
         * // reset the robot's x and y position
         * robot.resetLocalPosition();
